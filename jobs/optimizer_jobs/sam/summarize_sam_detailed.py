#!/usr/bin/env python3
"""
Detailed summary script for SAM optimizer results across all seeds.
...
def compare_with_baseline(sam_df, baseline_file):
    """Compare SAM per-class results with baseline per-class results."""
    ...
    for class_idx in sam_df['class']:
        sam_row = sam_df[sam_df['class'] == class_idx].iloc[0]
...
def main():
    parser = argparse.ArgumentParser(description="Summarize SAM results across seeds")
    parser.add_argument("--results_dir", 
                       default="train_results/sam", 
                       help="Directory containing training results")
    parser.add_argument("--output_file", 
                       default="evaluation_results/sam/per_class_stats.csv", 
                       help="Output CSV file for per-class summary")
    parser.add_argument("--comparison_file",
                       default="evaluation_results/sam/comparison_with_baseline.csv",
    ...
    for seed in seeds:
        ...
        eval_metrics_dir = Path("/home/scur1622/group_project/ai4mi_project/evaluation_results/sam/metrics") / f"seed_{seed}"
    ...
    print("\nSAM Per-Class Results (mean ± std over seeds):")
    print("="*60)
